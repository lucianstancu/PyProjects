...
class Game:
    ...
    def snake_out_of_map(self, x, y):
        if not (0 <= x <= 1000 and 0 <= y <= 800):
            return True
    ...
    def play(self):
        ...
        # sarpele mananca marul
        if self.is_collision(self.snake.x[0], self.snake.y[0], self.apple.x, self.apple.y):
            self.play_sound('ding')
            self.apple.move()
            self.snake.increase_length()

        # cand sarpele se loveste de el insusi
        for i in range(2, self.snake.length):
            if self.is_collision(self.snake.x[0], self.snake.y[0], self.snake.x[i], self.snake.y[i]):
                self.play_sound('crash')
                raise "Collision occurred"

        if self.snake_out_of_map(self.snake.x[0], self.snake.y[0]):
            self.play_sound('crash')
            raise "Snake hit the wall"
    ...
    def show_game_over(self):
        ...
        line2 = font.render("To play again press Enter. To quit press Esc ", True, (200, 200, 200))
