...
class ChatApplication:
    ...
    def run(self):
        self.window.mainloop()  # mainloop used in tkinter to start the application

    def _setup_main_window(self):
        self.window.title("Chat")  # title
        self.window.resizable(width=False, height=False)  # can't be modified
        self.window.configure(width=470, height=550, bg=BG_COLOR)  # dimensions and color
        ...
        # text widget (where the chat appears)
        ...
        self.scrollbar.place(relheight=1, relx=0.974)  # height = text_widget height, pos almost to right
        self.scrollbar.configure(command=self.text_widget.yview)  # moves the yposition of text_widget only
        ...
        self.msg_entry.focus()  # widget selected and start typing
        self.msg_entry.bind("<Return>", self._on_enter_pressed)  # be able to send message
